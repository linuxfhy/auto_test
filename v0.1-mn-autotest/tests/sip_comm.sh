...
function get_serviceip_midplane
{
    ...
    arr=($result)
    serviceip=$(printf "%d.%d.%d.%d" 0x${arr[0]} 0x${arr[1]} 0x${arr[2]} 0x${arr[3]})
    #serviceip="$((16#${arr[0]})).$((16#${arr[1]})).$((16#${arr[2]})).$((16#${arr[3]}))"
    echo $serviceip
}
...
function sip_switch_case
{
    ...
    local modeindex
    local cmcindex=2
    local cmcmode=("Active" "Standby" "Unknown")
    ...
    if [[ ${flag[0]} == 1 && ${flag[1]} == 0 ]]; then
        ...
        log_oak "cmd 1 mode [${cmcmode[$cmc0mode]}]" \
                "cmd 2 mode [${cmcmode[$cmc1mode]}]"
    fi
    echo $cmcindex
}
