...
ARCH=$(uname -p)
MACH=$(uname -p)
TESTS_PATH=${TESTS_PATH:-"./tests/demo"}

# SETUP and CLEANUP
SETUP=setup
CLEANUP=cleanup
g_setup="TRUE" # if g_setup == "FALSE", skip to run test cases

# DOMAKE (enable to run cases with suffix .sh)
g_domake=${DOMAKE:-"FALSE"}
...
function get_tc_list
{
	...
	if [[ $g_domake == "FALSE" ]]; then
		typeset tc_list="$(ls -1 $g_tests_path/tc*.sh)"
	else
		typeset tc_list="$(ls -1 $g_tests_path/tc* | grep -v ".sh$")"
	fi
	print "$tc_list"
}
...
function tests_end
{
	...
	(cd $RESULTS_DIR && ln -s $(basename $F_JNL) journal)
	(cd $RESULTS_DIR && ln -s $(basename $F_SUM) summary)
	print
...
function tests_setup
{
	...
	typeset setup=$g_tests_path/$SETUP
	[[ $g_domake == "FALSE" ]] && setup=${setup}.sh
	typeset tc=$setup
	typeset pid=$$
	typeset tc_res="N/A"
	typeset s_run=""
	...
	log_tc_start $pid $tc
	s_run=$(strL '.' 72 "Running root setup : $tc"); printf "\n$s_run"

	if [[ -f $setup ]]; then
		typeset f_log=$TMPDIR/$NAME.${func}.log.$$
		bash $setup > $f_log 2>&1
...
function tests_cleanup
{
	...
	typeset cleanup=$g_tests_path/$CLEANUP
	[[ $g_domake == "FALSE" ]] && cleanup=${cleanup}.sh
	typeset tc=$cleanup
	typeset pid=$$
	typeset tc_res="N/A"
	typeset s_run=""
	...
	log_tc_start $pid $tc
	s_run=$(strL '.' 72 "Running root cleanup : $tc"); printf "\n$s_run"

	if [[ -f $cleanup ]]; then
		typeset f_log=$TMPDIR/$NAME.${func}.log.$$
		bash $cleanup > $f_log 2>&1
		egrep -v "\[m$" $f_log | sed 's/^/stdout| /g' >> $F_JNL
		typeset res=$(egrep ".*XRES.*:.*" $f_log)
		tc_res=$(print $res | awk '{print $NF}')
	fi
...
function usage
{
	print "$1: [-h] [-t timeout] [-T tests dir] <-s|-c|-e> [TC]" >&2
	print "\t-h: help" >&2
...
g_remove=1
g_exitcode=0
g_tests_path=$TESTS_PATH

while getopts 'scet:T:rh' iopt; do
	case $iopt in
	s) g_init=0 ;;
	c) g_fini=0 ;;
	e) g_exec=0; g_init=0; g_fini=0 ;;
	t) g_timeout=$OPTARG ;;
	T) g_tests_path=$OPTARG ;;
	r) g_remove=0 ;;
