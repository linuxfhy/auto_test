...
function tc_start
{
	...
	export SH_FILE="$(get_abspath $1)"

	# extract assertion
	tc_assert $SH_FILE

	# save tc name which will be used by tc_xres()
	g_tc_name=$(basename $SH_FILE)
}
...
#
# DESCRIPTION
#	Get the absolute path of a file, like ${.sh.file} in ksh does
#
# RETURN
#	The return code from python -c <pycmd>
#
# NOTES
#	In bash, $0 and $BASH_SOURCE are not the absolute path of script
#
function get_abspath
{
	typeset file=${1?"*** file"}
	typeset pycmd=""
	pycmd+="import os.path; "
	pycmd+="print os.path.normpath(os.path.abspath(\"$file\"));"
	python -c "$pycmd"
	return $?
}
