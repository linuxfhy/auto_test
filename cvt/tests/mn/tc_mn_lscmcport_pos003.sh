...
function testLscmcport
{
	...
	typeset cmc_port_link=$($S_IMM_SERVICE -get_port_status | awk '{print $2}' \
		| sed -n ''$cmc_id'p')
	typeset portn_1=$(echo $cmc_port_link | cut -c 1-4)
	typeset portn_2=$(echo $cmc_port_link | cut -c 11-32)
	typeset portn_3=${portn_1}"0b0b0b"${portn_2}
	echo "portn_3=$portn_3"

	$S_IMM_SERVICE -set_port -target $data_cmc_1 -portn $portn_3 | \
		grep -q "set port complete"
	if (( $? != 0 )) ; then
		echo "set cmc $cmc_id port status down wrong, exit" >&2
		return 1
	fi
	RUN_NEU sleep 50
	$S_SVCINFO lscmcport | sed -n ''$data_cmc_2','$data_cmc_3'p' | \
		awk '{print $7}' | grep -q "up"
	if (( $? == 0 )); then
		echo "the cmc $cmc_id port S1/S2 status down wrong, exit" >&2
		return 1
	fi
	...
	$S_IMM_SERVICE -set_port -target $data_cmc_1 -portn $cmc_port_link | \
		grep -q "set port complete"
	if (( $? != 0 )) ; then
